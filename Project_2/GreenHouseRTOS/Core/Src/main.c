...
/* USER CODE BEGIN PFP */
void soilMoisture(void *pvParameters);
void tempHum(void *pvParameters);
void ldrMonitor(void *pvParameters);
...
int main(void)
{
  ...
  /* USER CODE BEGIN RTOS_THREADS */
	/* add threads, ... */
	x1Return = xTaskCreate(soilMoisture, "SOIL TASK",
	configMINIMAL_STACK_SIZE * 3,
	NULL, 2, &x1Handle);

	x2Return = xTaskCreate(tempHum, "T&H TASK",
	configMINIMAL_STACK_SIZE * 3,
	NULL, 1, &x2Handle);

	x3Return = xTaskCreate(ldrMonitor, "LIGHT TASK",
	configMINIMAL_STACK_SIZE * 3,
	NULL, 1, &x3Handle);
...
void tempHum(void *pvParameters) {
	for (;;) {
		if (DHT11_GetData(GPIOA, GPIO_PIN_2, &dht)) {
			...
			// ✅ Motor ON only if BOTH temperature >= 27 AND humidity >= 73
			if ((dht.Temperature >= 27 && dht.Humidity >= 73) && flag != 1) {
				uint16_t pwm_values[] = { 200, 400, 600 };
				for (int i = 0; i < 3; i++) {
			...
			// ✅ Motor OFF gradually if EITHER temp < 27 OR humidity < 73
			else if ((dht.Temperature < 27 && dht.Humidity < 73) && flag == 1) {
		...
		vTaskDelay(pdMS_TO_TICKS(100)); // Check every 2s
	}
}
...
void ldrMonitor(void *pvParameters) {
	for (;;) {
		...
		} else if (ldr_value >= 2500) {
			__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 50);
		} else if (ldr_value >= 1800) {
			__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 25);
		} else {
			__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 0);
		}

		vTaskDelay(pdMS_TO_TICKS(100)); // smoother update
	}
}
void configureTimerForRunTimeStats(void) {
	HAL_TIM_Base_Start(&htim12); // Timer already started
}
